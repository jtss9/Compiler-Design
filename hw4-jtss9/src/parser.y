...
extern char* source[];      /* declared in scanner.l */
extern uint32_t opt_dump;   /* declared in scanner.l */
...
int main(int argc, const char *argv[]) {
    ...
    SemanticAnalyzer sema_analyzer(source, opt_dump);
    root->accept(sema_analyzer);

    // TODO: do not print this if there's any semantic error
    if (sema_analyzer.has_error == false){
        printf("\n"
           "|---------------------------------------------------|\n"
